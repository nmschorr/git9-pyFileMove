...
ZacksTests logs onto Zacks.com - a financial website that tracks stocks and financial data. The username and password are stored in a properties file.  In order to run these tests, you need to create your own free account at Zacks.com and replace the values in the properties file with your own login and password. The properties file is named "Zacksprops.properties" in the "PropertiesFiles" project directory.  
...
The suite is comprised of two main java files - TestZacks.java and ZacksUtils.java.  The main tests are in TestZacks.java and the supporting code is in ZacksUtils.java.  The other supporting files are for interacting with the native interface - in this case - Windows 7.  A third small class file jnawin.java is there to work around the Mozilla crashing bug. 
